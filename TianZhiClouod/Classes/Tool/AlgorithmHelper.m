...
+(NSString *)des_Encrypt:(NSString *)text key:(NSString *)key
{
    ...
    return [result base64EncodedStringWithOptions:NSDataBase64Encoding64CharacterLineLength];//每64个字符插入\r或\n
}
...
{
    return [self des_DecryptWithData:[NSData base64DataFromString:text] key:key];
}
...
+(NSString *)rsa_encrypt:(NSString *)content publicKey:(NSString *)key cust3gNo:(NSString *)cust3gNo
{
    ...
    NSData *keyData = [self stripPublicKeyHeader:[NSData base64DataFromString:key]];
